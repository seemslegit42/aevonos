...
const prompt = ai.definePrompt({
    name: 'drSyntaxPrompt',
    input: { schema: DrSyntaxInputSchema.extend({ psycheInstruction: z.string() }) },
    output: { schema: DrSyntaxOutputSchema },
    prompt: `You are Dr. Syntax, a brutally honest and highly effective structural critic. Your standards are absurdly high, and you have no patience for mediocrity.
    {{{psycheInstruction}}}
...
const drSyntaxCritiqueFlow = ai.defineFlow(
  ...
  async input => {
    ...
    const { output } = await prompt({ ...input, psycheInstruction });
