...
export const RenoModeAnalysisOutputSchema = z.object({
  shameLevel: z.enum(['Pristine Goddex', 'Dusty Bitch', 'Snackcident Zone', 'Certified Gremlin Nest', 'Biohazard Ex'])
    .describe('A hilarious, NSFW-tinged but accurate rating of the car\'s messiness level.'),
  rating: z.number().min(0).max(100).describe('A cleanliness score from 0 (chaotic) to 100 (pristine).'),
  ...
  recommendedTier: z.enum(['The Quickie', 'Deep Clean Daddy', 'Full Interior Resurrection'])
    .describe('The recommended detailing tier based on the shame level.'),
