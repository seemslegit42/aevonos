...
async function main() {
  ...
  const userEmail = 'architect@aevonos.com';
  let user = await prisma.user.findUnique({
    where: { email: userEmail },
  });

  if (!user) {
    console.log('User not found, creating default user and workspace...');
    ...
    // Now seed the rest of the data since we know the user/workspace is new.
